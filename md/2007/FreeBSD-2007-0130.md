# tikiwiki -- multiple vulnerabilities
Secunia reports:

> Some vulnerabilities have been reported in TikiWiki, which can be
> exploited by malicious people to conduct cross-site scripting and
> script insertion attacks and disclose potentially sensitive
> information.
>
> Input passed to the username parameter in tiki-remind_password.php
> (when remind is set to send me my password) is not properly sanitised
> before being returned to the user. This can be exploited to execute
> arbitrary HTML and script code (for example with meta refreshes to a
> javascript: URL) in a user\'s browser session in context of an
> affected site.
>
> Input passed to the local_php and error_handler parameters in
> tiki-index.php is not properly verified before being used to include
> files. This can be exploited to include arbitrary files from local
> resources.
>
> Input passed to the imp_language parameter in
> tiki-imexport_languages.php is not properly verified before being used
> to include files. This can be exploited to include arbitrary files
> from local resources.
>
> Certain img src elements are not properly santised before being used.
> This can be exploited to insert arbitrary HTML and script code, which
> is executed in a user\'s browser session in context of an affected
> site when the malicious data is viewed.


## Affected packages
 - **tikiwik**
    - **Fixed:** 1.9.8.2

## Details
 - **published**: 2007-11-09T00:00:00Z
 - **modified**: 2008-10-03T00:00:00Z
 - **discovery**: 2007-08-27T00:00:00Z
 - **vid**: [20a4eb11-8ea3-11dc-a396-0016179b2dd5](https://vuxml.freebsd.org/freebsd/20a4eb11-8ea3-11dc-a396-0016179b2dd5.html)

## References
 - REPORT: [http://secunia.com/advisories/26618/](http://secunia.com/advisories/26618/)
 - ADVISORY: [https://api.osv.dev/v1/vulns/CVE-2007-4554](https://api.osv.dev/v1/vulns/CVE-2007-4554)
 - ADVISORY: [https://api.osv.dev/v1/vulns/CVE-2007-5683](https://api.osv.dev/v1/vulns/CVE-2007-5683)
 - ADVISORY: [https://api.osv.dev/v1/vulns/CVE-2007-5684](https://api.osv.dev/v1/vulns/CVE-2007-5684)
 - WEB: [http://secunia.com/advisories/26618/](http://secunia.com/advisories/26618/)
 - WEB: [http://tikiwiki.cvs.sourceforge.net/tikiwiki/tiki/changelog.txt?view=markup&pathrev=REL-1-9-8-2](http://tikiwiki.cvs.sourceforge.net/tikiwiki/tiki/changelog.txt?view=markup&pathrev=REL-1-9-8-2)
