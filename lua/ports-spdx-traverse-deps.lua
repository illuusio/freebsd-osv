...
local ports_spdx = require("ports-spdx")
...
local function spdx_traverse_describe_json(location)
	...
	local output_table = ports_make.target(addition_str .. "describe-json")
	local parser = ucl.parser()
	local _, err = parser:parse_string(output_table)

	if err ~= nil then
		return {}
	end
	local ucl_obj = parser:get_object()
	local rtn_table = {}
	...
	if ucl_obj["pkgbase"] == nil then
		...
		for _, cur_obj in pairs(ucl_obj) do
...
local function spdx_traverse_split_output(table_string)
	local splitted_table = ports_make.split_string(table_string, " ")
	local version_table = ports_make.split_string(splitted_table[1], "-")

	local version_table_len = #version_table
...
-- @return none
-------------------------------------------------------------------------------
local function spdx_traverse_add_depends_on(
	root_graph,
	deps_table,
	package,
	software_sbom,
	...
	for _, output_str in ipairs(deps_table) do
		...
		local to_spdx_id = ports_spdx.spdxId("software_Package", package_dep_table["name"])

		if depends_on_table == nil then
			depends_on_table = ports_spdx.add_relationship(
				root_graph,
				package.name,
				package.spdxId,
				to_spdx_id,
...
local agent, creation_info, spdx_document = ports_spdx.create_root(root_graph)

local output_table = ports_make.target_as_table("package-depends-list")
local package_data = spdx_traverse_describe_json()

-- Create SBOM object for package we are currently SBOMing
local software_sbom, package = ports_spdx.create_sbom(
	root_graph,
	package_data["name"],
...
spdx_traverse_add_depends_on(root_graph, output_table, package, software_sbom, spdx_document, creation_info)

-- Add depends to current package
for _, output_str in ipairs(output_table) do
	local package_dep_table = spdx_traverse_split_output(output_str)
	package_data = spdx_traverse_describe_json(package_dep_table["full_location"])
	local _, dep_package = ports_spdx.create_sbom(
		root_graph,
		package_data["name"],
		package_data["version"],
		package_data["license"],
	...
	local deps_table =
		ports_make.target_as_table("-C " .. package_dep_table["full_location"] .. " package-depends-list")
	spdx_traverse_add_depends_on(root_graph, deps_table, dep_package, software_sbom, spdx_document, creation_info)
end

-- Add licenses to Graph
ports_spdx.add_liceses(root_graph, spdx_document, creation_info)

local json_ld = ports_spdx.json_ld(root_graph)
